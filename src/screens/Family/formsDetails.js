...
const formsDetails = ({ navigation , route}) => {
  ...
  const getPermissionAndroid = async () => {
    try {
      ...
      alert(
        ...
        'Grant Me Permission to save file',
  ...
  const getTherapistProfile = () => {
    ...
    getApiHelper(
      ...
      if (response.code >= 200 && response.code <= 299) {
          //console.log('THEARPSIT ID ++++++',response.data)
  ...
  const getFamily = () => {
    ...
    getApiHelper(ServiceUrl.base_url_91 + ServiceUrl.getformsDetailbyFamilyID + route.params.familyMemberId, 'GET').then(
      (response) => {
        if (response.code >= 200 && response.code <= 299) {
          setData({
            ...data,
  ...
  const renderItem = ({item}) => (
    <View style={{flex: 1}}>
      <View style={{flexDirection: 'row', paddingHorizontal:10,paddingVertical:20}}>
        ...
        <View>
        {item.fileFormDto == null ? <View style = {{alignItems:'center',justifyContent:'center',flex:1, width: 90,backgroundColor:'#e8f1ff',borderRadius:10,
            height: 80}}>
        ...
        :
           ...
           (Platform.OS == 'ios')?
          <TouchableOpacity  onPress={() => downloadPdf(item.fileFormDto)}>
          <Image
              source={require('../../../assets/images/defaulFile.png')}
              style={{
                width: 80,
        ...
        <Image
            ...
            <View style={{flexDirection: 'row', marginTop: 10}}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                console.log(item)
                navigation.navigate('uploadForm', {Title : 'Update',
                familyMemberId:route.params.familyMemberId,
                sender:'ROLE_FAMILY',
                therapistId:route.params.therapistId,
                formID:item.fileFormDto.id,
                replyId:item.id})}}>
                <Image
                  source={require('../../../assets/images/upload.png')}
                  style={{
