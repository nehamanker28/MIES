...
const VideoCallFamily = ({ navigation, route }) => {
  var _engine
  ...
  //     }
  //   });
  //   // ApiHelper(ServiceUrl.base_url_91 + ServiceUrl.addAttachmentForCaseNote, data, 'POST')
  //   //     .then(response => {
  //   //         if (response.code >= 200 && response.code <= 299) {
      ...
      .then(response => {
        if (response.code >= 200 && response.code <= 299) {
          if (tempU.indexOf(response.data) === -1) {
            ...
            setUserDetail(tempU)
            if (response.data.userType == 'ROLE_THERAPIST') {
              setIsTherapistJoin(true)
            }
          }
        } else {
          ShowAlert(response.message)
  ...
  const sendEmoji = (img) => {
    ...
    setAnimated({
      ...animated,
      bool: true,
      //imgObj:[...animated.imgObj, {'img':animatedEmoji[img],'name':myName}],
  ...
  const receiveEmoji = (msg) => {
    setAnimated({
      ...animated,
      bool: true,
     // imgObj:[...animated.imgObj, {'img':animatedEmoji[msg.data.content],'name':msg.data.sender.name}],
    ...
    return (
      animated.bool ?
        <View style={{position: 'absolute', justifyContent: 'center', alignItems: 'center', alignSelf: 'center', top: '50%' }}>
          {/* {
            animated.imgObj.map((value) => {
              <View style={{backgroundColor:'gray', width:50, height:50}}>
                <Image source={value.img} style={{ width: 80, height: 80, resizeMode: 'contain', margin: 5 }}></Image>
                 <View style={{ backgroundColor: '#1667C9', padding: 5 }}>
                  ...
                  <Text style={{ fontFamily: 'Roboto-Regular', fontSize: 14, color: 'white', marginTop: 2 }}>{value.name}</Text>
                </View>  
              </View>
            })
          } */}
      ...
      <View style={styles.videoBaseView}>
        {
          isTherapistJoin ?
            ...
            <View style={{ flex: 1, backgroundColor: 'black', justifyContent: 'center', alignItems: 'center' }}>
              <Text style={{ fontFamily: 'Roboto-Regular', fontSize: 14, color: 'white' }}>Please wait for Therapist to start this session</Text>
            </View>
        }
        {renderLocalVideo()}
...
export default VideoCallFamily
