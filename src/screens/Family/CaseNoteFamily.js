...
const CaseNoteFamily = ({navigation,route}) => {
    ...
    const getCaseNotes = () => {
        ...
        getApiHelper(ServiceUrl.base_url_91 + ServiceUrl.getCaseNotes + '?queries='+ query +'&sort=startDateTime,DESC', 'GET')
        .then(response => {
    ...
    const renderItem = ({ item }) => (
        <TouchableOpacity style={{ flex: 1, marginVertical: 15 }} onPress={() => {
            //console.log('case note detail ====>',item)
                ...
                <View>
                    {
                        ...
                        <Text style={{ fontFamily: 'Roboto-Light', fontSize: 14 , marginRight:5}} numberOfLines= {2}>Pending translation</Text> //Submitted to Interpreter
                        :
                        item.caseNoteDTO.acceptCaseNotes ?
                        <Text style={{ fontFamily: 'Roboto-Light', fontSize: 14 , marginRight:5}}>Accepted</Text>
                        :
                        <Text style={{ fontFamily: 'Roboto-Light', fontSize: 14 , marginRight:5}}>Pending Acceptance</Text>
                    }
                    </View>:
                    <View>
                    {item.caseNoteDTO.acceptCaseNotes ?
