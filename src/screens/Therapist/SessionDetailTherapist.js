...
const SessionDetailTherapist = ({navigation, route}) => {
    ...
    useEffect(() => {
        if (isFocused) {
            //console.log('inttt ----------->>>>', route.params.session)
            var name = ''
            ...
            var fName = ''
            if(route.params.session.familyMember.firstName != null){
                fName = route.params.session.familyMember.firstName
            }
            if(route.params.session.familyMember.lastName != null){
                fName = fName + ' '+ route.params.session.familyMember.lastName
            }
            setFamilyName(fName)
    ...
    // const fetchCaseNoteByApi = () => {
    //     setData({
    //         ...data,
    //         isLoading: true
    //     })

    //     getApiHelper(ServiceUrl.base_url_91 + ServiceUrl.getCaseNoteBySession +'/'+ route.params.session.id, 'GET')
    //     .then(response => {
    //             if (response.code >= 200 && response.code <= 299) {
    //                 setData({
    //                     isLoading: false,
    //                     caseNote: response.data.description
    //                 })
    ...
    const onEdit = () => {
        navigation.push('Edit Session', {'session':route.params.session, 'isSessionDetail':true})
    }

    const onCancel = () => {
        //navigation.pop()
        navigation.push('CancelSessionTherapist', {'session':route.params.session})
    }
            ...
            <ScrollView style={styles.TopViewContainer}>
                ...
                <View style={{ flexDirection: 'row'}}>
                    <View style={{ flexDirection: 'column', flex:1 }}>
                        ...
                        <Text style={styles.text1}>Family</Text>
                        <Text style={styles.text2} numberOfLines={2}>{familyName}</Text>
                    </View>

                    <View style={{ flexDirection: 'column', flex:1 }}>
                        <Text style={styles.text11}>Status</Text>
                        {
                            (route.params.session.status === 'PENDING') ?
                                <Text style={styles.text21}>{item.status}</Text>
                                :
                                (route.params.session.status === 'UP_COMING') ?
                                    <Text style={styles.text21}>Up coming</Text>
                                    :
                                    (route.params.session.status === 'IN_PROGRESS') ?
                                        <Text style={styles.text21}>In Progress</Text>
                                        :
                                        (route.params.session.status === 'NOT_COMPLETED') ?
                                            <Text style={styles.text21}>Not Completed</Text>
                                            :
                                            (route.params.session.status === 'COMPLETED') ?
                                                <Text style={styles.text21}>Completed</Text>
                                                :
                                                (route.params.session.status === 'MISSED') ?
                                                    <Text style={styles.text21}>Missed</Text>
                                                    :
                                                    (route.params.session.status === 'CANCELLED') ?
...
export default SessionDetailTherapist
