...
const CaseNotesQueriesTherapist = ({route}) => {
...
const isFocused = useIsFocused();
  ...
  useEffect(() => {
    //console.log("route.params.session =======",route.params.session.queryDTO)
  ...
  //console.log('attach --->>>',route.params.session.caseNoteDTO.attachmentsList)
    if(route.params.session.queryDTO != null){
     // setPlaceholder("Enter your query")
      if (isFocused) {
        console.log('is focusedd --------->>>',route.params.session.queryDTO.attachmentsList)
       ...
       getApiHelper(
        ...
        if (response.code >= 200 && response.code <= 299) {
            setData({
              ...
              // console.log('QueriesReply :',data.QueryID)
  ...
  const downloadImage = async(imageURL) => {
    ...
    let DocumentDir = fs.dirs.PictureDir;
    let options = {
      fileCache: true,
      appendExt : ext,
      // addAndroidDownloads: {
  ...
  const renderItem1 = ({item}) => (
    <View style = {{padding:20,flexDirection:'row',marginTop:20,backgroundColor:colors.TEXTBOX_BLUE,borderRadius:20}}>
  ...
  const AddQueryReply = () =>{
    ...
    var formData = {
      'queryId': route.params.session.queryDTO.id,
      'reply':description,
              ...
              <View style={{ marginTop: 20 }}>
                ...
                <View style={{ marginTop: 20 }}>
                  ...
                  <View style={{ height: 38, marginTop: 30 }}>
                    <GreenButton text="Reply" onMethod={() => AddQueryReply()} />
                  </View>
                </View>
              </View>
            </View>
...
export default CaseNotesQueriesTherapist;
