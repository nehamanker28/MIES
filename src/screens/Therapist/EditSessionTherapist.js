...
const EditSessionTherapist = ({navigation, route}) => {
    ...
    const onSubmit = () => {
        ...
        let data = {
            "description": description,
             "familyMemberId": route.params.session.familyMember.id, 
             "interpreterId": interpreterId,
            "therapyId": route.params.session.therapy.id, 
            'startDateTime':fromTime,
            'endDateTime':toTime,
            'timeZone' : timeZone
        }

        ApiHelper(ServiceUrl.base_url_91 + ServiceUrl.updateTherapistSession +'/'+ route.params.session.id, data, 'PUT')
            .then(response => {
                setIsLoading(false)
                if (response.code >= 200 && response.code <= 299) {
                   // console.log('route.params.session.isSessionDetail ---->>',route.params.isSessionDetail)
                    if(route.params.isSessionDetail == null){
                        navigation.pop()
                    }else {
                        navigation.navigate('Session Details', {'session':response.data})
                    }
        ...
        <ScrollView keyboardShouldPersistTaps='handled' style={{ flex: 1 }}>
            ...
            <View style={styles.containerStyle}>
                ...
                <View style={{ paddingVertical: 20, height: 200 }}>
                    <GreenButton text='Update' onMethod={() => onSubmit()} />
...
export default EditSessionTherapist


// Edit session time error and past time should not select



// var hour = '0'
        // if (fromTime.getHours() < 10) {
        //     hour = '0'+fromTime.getHours()
        // } else {
        //     hour = fromTime.getHours()
        // }
        // var minute = '00'
        // if (fromTime.getMinutes() < 10) {
        //     minute = '0'+fromTime.getMinutes()
        // } else {
        //     minute = fromTime.getMinutes()
        // }

        // var endHour = '0'
        // if (toTime.getHours() < 10) {
        //     endHour = '0'+toTime.getHours()
        // }else {
        //     endHour = toTime.getHours()
        // }
        // var endMinute = '00'
        // if (toTime.getMinutes() < 10) {
        //     endMinute = '0'+toTime.getMinutes()
        // } else {
        ...
        // let startDate = moment.utc(fromTime).format('YYYY-MM-DD')
        // let endDate = moment.utc(toTime).format('YYYY-MM-DD')
        
        // let startTime = moment.utc(fromTime).format('HH:mm:ss')
        // let endTime = moment.utc(toTime).format('HH:mm:ss')
