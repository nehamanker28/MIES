...
import { View, Text, FlatList, StyleSheet, Image, TouchableOpacity, SafeAreaView, RefreshControl } from "react-native";
...
const UpComingSessionTherapist = (props) => {
    ...
    const onPressEdit = (item) => {
        ...
        props.navigation.push('Edit Session', {'session':item})
    }

    const onPressDelete = (item) => {
        props.navigation.push('CancelSessionTherapist', {'session':item})
        // props.navigation.push('DeleteSession', {'session':item})
    }
    ...
    const onAddNewSection = () => {
       // //console.log('add sessions')
        props.navigation.push('Add Session', {'test':selectedDate})
    }
   ...
   const getSessionsByDate = (date) => {
       ...
       // //console.log('endDay sub------>>>',endDay)
        ...
        getApiHelper(ServiceUrl.base_url_91 + ServiceUrl.getSessionForTherapist + '?startFromDateTime=' + sTime +'&startToDateTime='+ eTime + '&timeZone='+ timeZone + '&sort=startDateTime,ASC', 'GET')
        .then(response => {
            if (response.code >= 200 && response.code <= 299) {
                    ...
                    response.data.content.map((item, index) => {
                        ...
                        if (time <= 5 && endTime >= 1 && item.status != 'COMPLETED' && item.status != 'CANCELLED') {
                            ////console.log('is item ----->>',response.data.status)
    ...
    const onShowDetail = (item) => {
        ...
        props.navigation.push('Session Details', {'session':item})
    }
